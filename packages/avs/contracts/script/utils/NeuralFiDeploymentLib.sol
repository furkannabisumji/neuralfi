...
import {NeuralFiServiceManager} from "../../src/NeuralFiServiceManager.sol";
...
library NeuralFiDeploymentLib {
    ...
    struct DeploymentData {
        address neuralFiServiceManager;
    ...
    function deployContracts(
        ...
        // Deploy proxies
        result.neuralFiServiceManager = UpgradeableProxyLib.setUpEmptyProxy(proxyAdmin);
        result.stakeRegistry = UpgradeableProxyLib.setUpEmptyProxy(proxyAdmin);
        
        // Deploy implementation contracts
        address stakeRegistryImpl = address(new ECDSAStakeRegistry(IDelegationManager(core.delegationManager)));
        address neuralFiServiceManagerImpl = address(
            new NeuralFiServiceManager(
                core.avsDirectory, 
        ...
        bytes memory upgradeCall = abi.encodeCall(
            ...
            (result.neuralFiServiceManager, 0, quorum)
        );
        UpgradeableProxyLib.upgradeAndCall(result.stakeRegistry, stakeRegistryImpl, upgradeCall);

        upgradeCall = abi.encodeCall(
            NeuralFiServiceManager.initialize, 
            (owner, rewardsInitiator)
        );
        UpgradeableProxyLib.upgradeAndCall(result.neuralFiServiceManager, neuralFiServiceManagerImpl, upgradeCall);
    ...
    function readDeploymentJson(
        ...
        require(vm.exists(fileName), "NeuralFiDeployment: Deployment file does not exist");

        string memory json = vm.readFile(fileName);
        DeploymentData memory data;
        
        data.neuralFiServiceManager = json.readAddress(".addresses.neuralFiServiceManager");
    ...
    function writeDeploymentJson(DeploymentData memory data) internal {
        writeDeploymentJson("deployments/neuralfi/", block.chainid, data);
    }
    ...
    function writeDeploymentJson(
        ...
        address proxyAdmin = address(UpgradeableProxyLib.getProxyAdmin(data.neuralFiServiceManager));
    ...
    function readDeploymentConfigValues(
        ...
        require(vm.exists(pathToFile), "NeuralFiDeployment: Deployment Config file does not exist");
    ...
    function _generateContractsJson(
        ...
        return string.concat(
            ...
            '","neuralFiServiceManager":"',
            data.neuralFiServiceManager.toHexString(),
            '","neuralFiServiceManagerImpl":"',
            data.neuralFiServiceManager.getImplementation().toHexString(),
            '","stakeRegistry":"',
