...
contract SetupPayments is Script, Test {
    ...
    function setUp() public {
        ...
        neuralFiDeployment = NeuralFiDeploymentLib.readDeploymentJson("deployments/hello-world/", block.chainid);
        neuralFiConfig = NeuralFiDeploymentLib.readDeploymentConfigValues("config/hello-world/", block.chainid);
    ...
    function run() external {
        vm.startBroadcast(neuralFiConfig.rewardsInitiatorKey);
    ...
    function executeProcessClaim() public {
        ...
        vm.startBroadcast(deployer);
        earnerLeaves = _getEarnerLeaves(_getEarners(deployer), amountPerPayment, neuralFiDeployment.strategy);
    ...
    function createAVSRewardsSubmissions(uint256 numPayments, uint256 amountPerPayment, uint32 startTimestamp) public {
        ERC20Mock(neuralFiDeployment.token).mint(neuralFiConfig.rewardsInitiator, amountPerPayment * numPayments);
        ERC20Mock(neuralFiDeployment.token).increaseAllowance(neuralFiDeployment.neuralFiServiceManager, amountPerPayment * numPayments);
        uint32 duration = rewardsCoordinator.MAX_REWARDS_DURATION();
        SetupPaymentsLib.createAVSRewardsSubmissions(
            neuralFiDeployment.neuralFiServiceManager,
            neuralFiDeployment.strategy,
            numPayments,
            amountPerPayment,
            duration,
            startTimestamp
